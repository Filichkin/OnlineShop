...
class Settings(BaseSettings):
    ...
    def validate_secret(cls, value: str, info: ValidationInfo) -> str:
        ...
        if not value or value == 'SECRET':
            raise ValueError(
                'SECRET must be set in environment variables. '
                'Do not use default value.'
            )

        if len(value) < Constants.SECRET_MIN_LENGTH:
            raise ValueError(
    ...
    def validate_postgres_password(
        ...
        if len(value) < Constants.DB_PASSWORD_MIN_LENGTH:
            raise ValueError(
    ...
    def validate_superuser_password(
        ...
        if len(value) < Constants.SUPERUSER_PASSWORD_MIN_LENGTH:
            raise ValueError(
